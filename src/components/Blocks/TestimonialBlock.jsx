...
const TestimonialBlock = ({ block, onSelect, isPreview }) => {
  ...
  const stars =
    "★".repeat(props.rating || 5) + "☆".repeat(5 - (props.rating || 5));
  ...
  return (
    <div style={containerStyle} onClick={handleClick}>
      {props.authorImage && (
        <img src={props.authorImage} alt="Avatar" style={avatarStyle} />
      )}
      ...
      <div>
        ...
        <div style={titleStyle}>
          {props.authorTitle || "Marketing Director"}
        </div>
      </div>
      {props.rating && (
        <div className="mt-2 text-yellow-500 text-lg">{stars}</div>
      )}
    </div>
  );
};
