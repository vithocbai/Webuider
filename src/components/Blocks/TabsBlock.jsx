...
const TabsBlock = ({ block, onSelect }) => {
  ...
  const tabButtonStyle = (index) => ({
    ...
    border: "none",
    backgroundColor:
      activeTab === index ? props.activeTabBgColor || "#fff" : "transparent",
    color:
      activeTab === index
        ? props.activeTabTextColor || "#007bff"
        : props.tabTextColor || "#555",
  ...
  return (
    <div style={tabsContainerStyle} onClick={handleClick}>
      {tabs.length > 0 ? (
        <>
          ...
          <div style={tabContentStyle}>
            <ReactMarkdown>
              {tabs[activeTab]?.content || "No content for this tab."}
            </ReactMarkdown>
          </div>
        </>
      ) : (
